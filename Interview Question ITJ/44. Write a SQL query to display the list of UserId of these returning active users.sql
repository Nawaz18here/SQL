--44. Write a SQL query to display the list of UserId of these returning active users.
...
With CTE as (
Select *,LEAD(CreatedAt,1) Over(Partition by UserID Order by CreatedAt ) as LEADDate from Transactions_Amazon )
, CTE2 as (
Select *,DATEDIFF(DD,CreatedAt,LEADDate) as DiffDate from CTE )
Select DISTINCT USERID from CTE2 where DiffDate<=7
